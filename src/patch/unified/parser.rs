...
pub enum ParseError {
    ...
    #[fail(display = "Invalid permissions: \"{}\"", permissions_str)]
    BadPermissions { permissions_str: String },
...
impl<'a> From<nom::Err<CompleteByteSlice<'a>>> for ParseError {
    fn from(err: nom::Err<CompleteByteSlice<'a>>) -> ParseError {
        ...
        match code {
            ...
            c if c == ParseErrorCode::BadPermissions as u32 => {
                ParseError::BadPermissions { permissions_str: place_as_string }
            }
            _ => {
...
fn parse_permissions(input: CompleteByteSlice) -> IResult<CompleteByteSlice, Permissions> {
    let (input, _) = take_while!(input, is_space)?;
    let (input, digits) = take_while1!(input, is_oct_digit)?;

    if digits.len() != 6 { // This is what patch requires, but otherwise it fallbacks to Permissions(0), so maybe we should too?
        return Err(nom::Err::Failure(error_position!(input, nom::ErrorKind::Custom(ParseErrorCode::BadPermissions as u32))));
    }

    let permissions_str = std::str::from_utf8(&digits).unwrap(); // NOTE(unwrap): We know it is just digits 0-7, so it is guaranteed to be valid UTF8.
    match u32::from_str_radix(permissions_str, 8) {
        Ok(number) => Ok((input, Permissions(number))),
        Err(_) => Err(nom::Err::Failure(error_position!(input, nom::ErrorKind::Custom(ParseErrorCode::BadPermissions as u32)))),
    }
}
...
fn test_parse_permissions() {
    assert_parsed!(parse_permissions, b"123456", Permissions(0o123456));
    assert_parsed!(parse_permissions, b"012345", Permissions(0o12345));
    assert_parsed!(parse_permissions, b"   123456", Permissions(0o123456));
    assert_parsed!(parse_permissions, b"   012345", Permissions(0o12345));

    assert_parsed!(parse_permissions, b"100755", Permissions(0o100755));
    assert_parsed!(parse_permissions, b"100644", Permissions(0o100644));

    assert_parse_error_code!(parse_permissions, b"100aaa", ParseErrorCode::BadPermissions as u32);
    assert_parse_error_code!(parse_permissions, b"1", ParseErrorCode::BadPermissions as u32);
    assert_parse_error_code!(parse_permissions, b"10000000", ParseErrorCode::BadPermissions as u32);
    assert_parse_error_code!(parse_permissions, b"1000000000000000000000000000", ParseErrorCode::BadPermissions as u32);
}
...
enum MetadataLine {
    ...
    OldMode(Permissions),
    NewMode(Permissions),
    DeleteFileMode(Permissions),
    NewFileMode(Permissions),
...
named!(parse_metadata_line<CompleteByteSlice, MetadataLine>,
    alt!(
        ...
        do_parse!(tag!(s!(b"old mode "))         >> permissions: parse_permissions >> newline >> (MetadataLine::OldMode(permissions))) |
        do_parse!(tag!(s!(b"new mode "))         >> permissions: parse_permissions >> newline >> (MetadataLine::NewMode(permissions))) |
        do_parse!(tag!(s!(b"delete file mode ")) >> permissions: parse_permissions >> newline >> (MetadataLine::DeleteFileMode(permissions))) |
        do_parse!(tag!(s!(b"new file mode "))    >> permissions: parse_permissions >> newline >> (MetadataLine::NewFileMode(permissions)))
    )
);
...
impl FilePatchMetadata {
    ...
    pub fn build_filepatch<'a>(self, hunks: &[TextHunk]) -> Option<TextFilePatch<'a>> {
        let kind = self.recognize_kind(hunks);
        ...
        match (self.old_filename, self.new_filename) {
            ...
            (Some(Filename::Real(filename)), _)
             => {
                Some(TextFilePatch::new(kind, filename))
            },
            _ => {
                None
            }
    ...
    pub fn build_renaming_filepatch<'a>(self, hunks: &[TextHunk]) -> Option<TextFilePatch<'a>> {
        let kind = self.recognize_kind(hunks);

        if let (Some(Filename::Real(new_filename)), Some(Filename::Real(old_filename))) =
            (self.new_filename, self.old_filename)
        {
            Some(TextFilePatch::new_renamed(kind, old_filename, new_filename))
        } else {
            None
        }
    }
...
fn parse_filepatch<'a>(mut input: CompleteByteSlice<'a>) -> IResult<CompleteByteSlice, TextFilePatch<'a>> {
    ...
    loop {
        ...
        match patch_line {
            ...
            EndOfPatch | Metadata(MetadataLine::GitDiffSeparator(..)) => {
                ...
                // could be still valid rename patch, so lets check for that.
                if metadata.old_filename.is_some() &&
                  metadata.new_filename.is_some() &&
                  metadata.rename_from && metadata.rename_to
                {
                    ...
                    return Ok((input, tmp_metadata.build_renaming_filepatch(&[]).unwrap())); // NOTE(unwrap): It must succeed since we have the filenames.
                } else {
    ...
    let filepatch = if metadata.rename_from && metadata.rename_to {
        metadata.build_renaming_filepatch(&hunks)
    } else {
        metadata.build_filepatch(&hunks)
    };

    let mut filepatch = filepatch.ok_or_else(
        || nom::Err::Failure(error_position!(input, nom::ErrorKind::Custom(ParseErrorCode::MissingFilenameForHunk as u32)))
    )?;

    filepatch.hunks = hunks;
